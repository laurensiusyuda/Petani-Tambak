...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  AuthBloc({required this.authRepository}) : super(UnAuthenticated()) {
    ...
    on<ForgotPassword>((event, emit) async {
      emit(Loading());
      try {
        await authRepository.resetPasswordEmail(email: event.email);
        emit(Authenticated());
      } catch (e) {
        emit(AuthError(e.toString()));
        emit(UnAuthenticated());
    ...
    // When User Presses the Forgot Password Button, we will send the VericationEmailRequested Event to the AuthBloc to handle it and emit the UnAuthenticated State
